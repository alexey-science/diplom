    ...
    <div class="info-block">
        ...
        <div class="linkinfo">
            ...
            <?php foreach ($si['gs'] as $item): ?>
                <div class="link-row">
                    <div style="width: 30px; height: 30px; background-color: #000;">
                        <a href="?delItem&type=<?=$type?>&service=gs&sid=<?=$item['id']?>" style="color: #fff;">X</a>
                    </div>
                    <a href="?getStat&type=<?=$type?>&service=gs&sid=<?=$item['id']?>"><?=strip_tags($item['name']);?> </a>Дата: <?=strip_tags($item['dateq']);?> </div>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
        <div class="linkinfo">
            ...
            <?php foreach ($si['elib'] as $item): ?>
                <div class="link-row">
                    <div style="width: 30px; height: 30px; background-color: #000;">
                        <a href="?delItem&type=<?=$type?>&service=elib&sid=<?=$item['id']?>" style="color: #fff;">X</a>
                    </div>
                    <a href="?getStat&type=<?=$type?>&service=elib&sid=<?=$item['id']?>"><?=strip_tags($item['name']);?> </a>
                    Дата: <?=strip_tags($item['dateq']);?>
                </div>
            <?php endforeach; ?>
        </div>
        <div class="linkinfo">
            ...
            <?php foreach ($si['clen'] as $item): ?>
                ...
                <div class="link-row">
                    <div style="width: 30px; height: 30px; background-color: #000;">
                        <a href="?delItem&type=<?=$type?>&service=clen&sid=<?=$item['id']?>" style="color: #fff;">X</a>
                    </div>
                    <a href="?getStat&type=<?=$type?>&service=clen&sid=<?=$item['id']?>"><?=strip_tags($item['name']);?> </a> Дата: <?=strip_tags($item['dateq']);?> </div>
            <?php endforeach; ?>
        </div>
    </div>
</div>